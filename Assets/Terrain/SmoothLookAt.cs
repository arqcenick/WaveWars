    ...
    void LateUpdate()
    {
        if (target)
        {
            if (smooth)
            {
                ...
                _myTransform.rotation = Quaternion.Slerp(_myTransform.rotation, rotation, Time.deltaTime * 100f);
            }
            else
            { //Just look at
                ...
                {
                    alpha = Mathf.Lerp(alpha, 0.0f, Time.deltaTime * 10.0f);
                }
                else
                {
                    alpha = Mathf.Lerp(alpha, 1.0f, Time.deltaTime * 10.0f);
